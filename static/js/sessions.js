...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'GodSpeed Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to GodSpeed kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'GodSpeed Chat';
    }
